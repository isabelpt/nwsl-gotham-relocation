const MODELS = [
  {
    ...
    role: "One combined model (linear stage + CatBoost residual), fit on a random 80% of rows. A venue's other games can leak into its own held-out fold, so this is the model behind the optimistic-looking r = 0.883.",
    use: 'Diagnostic only — discarded after use',
    accent: false,
  },
  {
    tag: '2',
    name: 'LOVO (22 throwaway models)',
    role: 'Refit once per (team, venue) pair, that pair fully excluded each time, tested only on the held-out venue. This is the honest generalization check — pooled r = 0.126. Replaced team-level LOTO on 2026-08-25: Gotham is always in-sample as a team, so the real question is whether the model handles an unfamiliar venue, not an unfamiliar team.',
    use: 'Diagnostic only — discarded after use',
    accent: false,
  },
  {
    tag: '3',
    name: 'final_lin + final_cb',
    role: 'Fit once on every available row, Gotham included. Not tested on a held-out venue — this is the model actually used to answer the question. The linear stage extrapolates Etihad Park\'s accessibility directly (log-transformed, no capping); CatBoost only ever sees in-range features.',

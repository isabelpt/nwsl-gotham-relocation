...
import maplibregl, { Map as MapLibreMap, ExpressionSpecification, StyleSpecification } from "maplibre-gl";
import "maplibre-gl/dist/maplibre-gl.css";
import { ClusterInfo, MapView, SITE_CONFIG, SiteLabel, Threshold } from "./types";
...
const BASEMAP_STYLE: StyleSpecification = {
  ...
  sources: {
    "carto-light": {
      type: "raster",
      // MapLibre's raster "tiles" templates only substitute {z}/{x}/{y} (no {r}/{ratio} like
      // Leaflet) -- one URL per subdomain, standard (non-retina) resolution.
...
/** Fill-color: categorical, one flat color per cluster id (matches 08's static map palette).
 * The fallback color is never actually seen -- unclustered tracts are made fully transparent
 * by clusterOpacityExpr below instead of painted gray. */
function clusterColorExpr(clusters: ClusterInfo[]): ExpressionSpecification {
  const stops = clusters.flatMap((c) => [c.id, c.color]);
  return ["match", ["coalesce", ["get", "cluster"], -1], ...stops, "#000000"] as unknown as ExpressionSpecification;
}

/** Fill-opacity: hide tracts code/supplementary/08_tract_clustering.ipynb didn't assign a cluster to, instead of
 * filling them with a gray "unclustered" color. */
function clusterOpacityExpr(): ExpressionSpecification {
  return ["case", ["==", ["get", "cluster"], null], 0, 0.75] as unknown as ExpressionSpecification;
}

function fillColorExpr(view: MapView, clusters: ClusterInfo[]): ExpressionSpecification {
  // A "match" expression needs at least one label/output pair before the fallback, so fall
  // back to the compare coloring if the cluster legend hasn't loaded yet (race with the
  // map's own async load handler -- resolved a moment later by the update effect below).
  return view === "clusters" && clusters.length > 0 ? clusterColorExpr(clusters) : compareColorExpr();
}
function fillOpacityExpr(view: MapView, threshold: Threshold): ExpressionSpecification | number {
  return view === "clusters" ? clusterOpacityExpr() : compareOpacityExpr(threshold);
}
...
const IsochroneMap = forwardRef<IsochroneMapHandle, Props>(function IsochroneMap(
  { view, threshold, clusters, onTractHover },
  ref
) {
  ...
  // Initial map setup -- runs once. Reads `view`/`threshold`/`clusters` only for the first
  // paint; later changes are applied by the effect below via setPaintProperty.
  useEffect(() => {
    ...
    map.on("load", async () => {
      ...
      map.addLayer({
        ...
        source: "tracts",
        paint: {
          "fill-color": fillColorExpr(view, clusters),
          "fill-opacity": fillOpacityExpr(view, threshold),
        },
      });
  ...
  // Update fill styling when view/threshold/clusters change.
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !loadedRef.current || !map.getLayer("tracts-fill")) return;
    map.setPaintProperty("tracts-fill", "fill-color", fillColorExpr(view, clusters));
    map.setPaintProperty("tracts-fill", "fill-opacity", fillOpacityExpr(view, threshold));
  }, [view, threshold, clusters]);
  ...
  useImperativeHandle(ref, () => ({
    async searchAddress(query: string) {
      ...
      // moveend fires once the camera animation ends, not necessarily after that viewport's
      // tiles have actually painted -- queryRenderedFeatures only sees painted tiles, so wait
      // for one more idle render pass before querying (otherwise it can come back empty).

...
Usage:
    python3 10_isochrones_new_venues.py <metro_key>
...
SPEED_WALKING_KMH = 5.0  # see README "Walking speed correction" -- r5py default (3.6) is unrealistically slow
...
def main(metro_key):
    ...
    if missing:
        raise FileNotFoundError(f"{log_prefix} missing inputs, run 09_gtfs_pull_new_venues.ipynb first: {missing}")
...
if __name__ == "__main__":
    if len(sys.argv) != 2 or sys.argv[1] not in METROS:
        print(f"Usage: python3 10_isochrones_new_venues.py <metro_key>")
